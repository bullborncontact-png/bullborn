...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Attend une promesse au maximum "ms" millisecondes. Si elle ne
  // répond pas à temps (Firebase mal configuré, bloqué par le réseau,
  // hors-ligne...), on n'attend pas indéfiniment : on continue avec
  // "null" pour que la commande WhatsApp parte quand même.
  ...
  if (checkoutForm) {
    checkoutForm.addEventListener('submit', (e) => {
      ...
      // cas, en cas d'erreur, ou si ça ne répond pas sous 6 secondes
      // (réseau bloqué, Firebase mal configuré...), la commande
      // WhatsApp part quand même — seul le suivi ne sera pas dispo.
      const rawPromise = (window.BullbornTracking && typeof BullbornTracking.createOrder === 'function')
        ? BullbornTracking.createOrder(prenom).catch(() => null)
        : Promise.resolve(null);
      const trackingPromise = withTimeout(rawPromise, 6000);
