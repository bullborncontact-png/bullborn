...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (checkoutForm) {
    checkoutForm.addEventListener('submit', (e) => {
      ...
      // Enregistre la commande dans Firebase (voir firebase-config.js)
      // pour qu'elle apparaisse dans admin.html avec un statut à jour.
      // Si Firebase n'est pas configuré, ça ne bloque rien : la commande
      // part quand même sur WhatsApp comme avant.
      ...
      setTimeout(() => {
        if (typeof db !== 'undefined') {
          ...
          }).then(goToWhatsapp).catch(goToWhatsapp);
        } else {
          goToWhatsapp();
        }
      }, 450);
