...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------- Suivi de commande (Firebase) ----------
     Génère un code de suivi unique (ex: BB-4F7K2Q) et enregistre
     la commande dans Firebase Realtime Database sous orders/{code}
     pour qu'elle soit consultable depuis suivi.html, mise à jour
     depuis admin.html, et suivie en GPS depuis livreur.html.
     Si firebase-config.js n'est pas configuré, cette partie est
     simplement ignorée et le site continue de fonctionner comme
     avant (commande envoyée uniquement par WhatsApp). */
  ...
  function saveOrderToFirebase(code, orderData) {
    if (!isFirebaseReady()) return Promise.resolve(false);
    return firebase.database().ref('orders/' + code).set(orderData)
      .then(() => true)
      .catch((err) => {
        console.error('BULLBORN: échec de l\'enregistrement de la commande dans Firebase', err);
        return false;
      });
  }
  ...
  const trackingCloseBtn2 = document.getElementById('trackingCloseBtn2');
  if (trackingCloseBtn2) {
    trackingCloseBtn2.addEventListener('click', () => {
      closeCheckout();
      // Remet le modal dans son état initial pour la prochaine commande.
      setTimeout(() => {
        const trackingSuccess = document.getElementById('trackingSuccess');
        if (trackingSuccess) trackingSuccess.classList.remove('show');
  ...
  if (checkoutForm) {
    checkoutForm.addEventListener('submit', (e) => {
      ...
      const orderData = {
        ...
        items: cart,
        total,
        status: 'recue', // recue -> preparation -> livraison -> livree
        location: null,
        createdAt: Date.now()
      };
      ...
      setTimeout(() => {
        saveOrderToFirebase(trackingCode, orderData).finally(() => {
          ...
          // Affiche le code de suivi au client à la place du formulaire.
          const trackingCodeValue = document.getElementById('trackingCodeValue');
          const trackingSuccess = document.getElementById('trackingSuccess');
          if (trackingCodeValue) trackingCodeValue.textContent = trackingCode;
          if (trackingSuccess) trackingSuccess.classList.add('show');
