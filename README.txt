...
8. SUIVI DES COMMANDES (page admin)
---------------------------------------
Une page admin.html a été ajoutée : chaque commande passée depuis le
site (bouton "Confirmer la commande") est maintenant enregistrée dans
une base de données Firebase (gratuite), en plus de partir sur
WhatsApp comme avant. Depuis admin.html, tu vois toutes les commandes
en direct et tu peux changer leur statut : En attente / Confirmée /
En préparation / Livrée / Annulée — ça se sauvegarde automatiquement.

Pour l'activer (5 minutes, gratuit) :
  1. Ouvre firebase-config.js et suis les instructions écrites en
     haut du fichier (créer un projet Firebase + Realtime Database).
  2. Colle tes identifiants Firebase dans ce même fichier.
  3. Ouvre admin.html dans le navigateur, connecte-toi avec le mot
     de passe défini dans admin.html (ADMIN_PASSWORD, tout en bas
     du fichier — change-le pour un mot de passe à toi).
  4. Passe une commande test sur le site : elle doit apparaître
     dans admin.html en quelques secondes.

À savoir :
  - Tant que firebase-config.js n'est pas rempli, le site fonctionne
    normalement (WhatsApp), les commandes ne sont juste pas
    enregistrées dans admin.html.
  - Le mot de passe d'admin.html protège l'accès à la page, mais pas
    la base de données elle-même (voir la note sécurité dans
    firebase-config.js) — ne partage jamais le lien admin.html
    publiquement.
  - N'oublie pas d'uploader firebase-config.js et admin.html en
    même temps que le reste du site sur ton hébergeur.
